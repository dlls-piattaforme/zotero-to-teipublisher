...
########################################## MONOGR ############################################   
    ...
    if item.get('title'):
        ET.SubElement(monogr, "title", level="m").text = item['title'] #level m (rende cliccabile i titoli su TEI Publisher)
    ...
    if item.get('series') or item.get('seriesTitle'):
        ...
        if item.get('series'):
            #replaceFormatting
            ET.SubElement(series, "title", level='s').text = item['series'] #level='s': immagini openseadragon
...
# 	// create the imprint
    ...
    if item.get('archiveLocation'):
        #divide archiveLocation e archiveLibrary attraverso il separatore ': '
        ...
        if len(archiveData) > 1:
            ...
            ET.SubElement(imprint, "biblScope", unit='archiveLocation' ).text = item['archiveLocation'] #se non trova il separatore mette tutto dentro qui
    ...
    #childNodes[0] to remove first line <?xml version="1.0" ?>
